...
def train_with_self_attn(
        train_dataloader, encoder, decoder, n_epochs, logger, output_lang, input_lang, learning_rate=0.001,
    ...
    decoder_optimizer = optim.Adam(decoder.parameters(), lr=learning_rate)
    criterion = nn.NLLLoss()
    self_att = SelfAttention(128, 128).to(device)

    for epoch in range(1, n_epochs + 1):
        loss = train_epoch(train_dataloader, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion,output_lang, self_att)
        print_loss_total += loss
        plot_loss_total += loss
...
def main():
    wandb_logger = Logger(f"inm706_translation_attention_bahdanau", project='inm706_lab5')
    ...
    decoder_bahdanau_attention = AttnDecoderGRU(hidden_size, dataset.output_lang.n_words).to(device)
    decoder_luong_attention = AttnDecoderGRU(hidden_size, dataset.output_lang.n_words, attention_type='luong').to(device)
    decoder = DecoderRNN(hidden_size, dataset.output_lang.n_words).to(device)

    #train(train_dataloader, encoder, decoder_bahdanau_attention, n_epochs, logger, dataset.output_lang, print_every=5, plot_every=5, plot_attention=True)
    #train(train_dataloader, encoder, decoder_luong_attention, n_epochs, logger, dataset.output_lang, print_every=5, plot_every=5, plot_attention=False)

    train_with_self_attn(train_dataloader, encoder,decoder, n_epochs, logger, dataset.input_lang, dataset.output_lang, print_every=5, plot_every=5)
    return
